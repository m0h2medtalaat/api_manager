...
class PostRequestMultiPartMock with Request, PostRequest {
  PostRequestMultiPartMock(this.requestModel);
...
void main() {
  ...
  group('Base Request', () {
    ...
    test('should contain the header', () async {
      expect(
          request.headers, headers..addAll({'Authorization': 'Bearer $token'}));
    });
  ...
  group('Post Request', () {
    ...
    late RequestModelMock requestModel;
    late PostRequestMultiPartMock multiPartMock;

    setUp(() {
      requestModel = RequestModelMock(data: 'data_mock');
      postRequest = PostRequestMock(requestModel);
      multiPartMock = PostRequestMultiPartMock(requestModel);
    });
    ...
    test('data should be equest Model.toMap', () async {
      expect(await postRequest.data, await requestModel.toMap());
    });

    test('multipart data should be equest Model.toMap', () async {
      expect((await multiPartMock.data as FormData).fields.first.value,
